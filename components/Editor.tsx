...
import { PixelArt, Layer, ToolType, SymmetryMode, ThemeType, Language } from '../types';
...
import { 
  ...
  Languages, Image as ImageIcon, Upload
} from 'lucide-react';
...
const translations = {
  zh: {
    tools: { pen: '画笔', del: '擦除', fill: '填充', pick: '吸色', pan: '移动' },
    undo: '撤销',
    redo: '重做',
    res: '分辨率',
    save: '保存',
    ...
    sym: '对称',
    status: '系统反馈',
    exportUI: '导出 UI 资源 (DEV)',
    importLayer: '导入至当前图层'
  },
  en: {
    tools: { pen: 'PEN', del: 'DEL', fill: 'FILL', pick: 'PICK', pan: 'PAN' },
    undo: 'UNDO',
    redo: 'REDO',
    res: 'RES',
    save: 'SAVE',
    ...
    sym: 'SYM',
    status: 'SYSTEM_FEED',
    exportUI: 'EXPORT UI ASSETS (DEV)',
    importLayer: 'IMPORT TO LAYER'
  }
};
...
export const Editor: React.FC<EditorProps> = ({ art, onBack, currentTheme, onSetTheme }) => {
  ...
  const handleImportToLayer = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          tempCanvas.width = art.width;
          tempCanvas.height = art.height;
          ...
          if (ctx) {
            ...
            ctx.drawImage(img, sx, sy, minDim, minDim, 0, 0, art.width, art.height);
            
            const imageData = ctx.getImageData(0, 0, art.width, art.height).data;
            ...
            for (let i = 0; i < imageData.length; i += 4) {
              ...
              if (a < 128) {
                ...
                pixelData.push("#" + ((1 << 24) + (r << 16) + (g << 8) + b).toString(16).slice(1));
              }
            }
            
            const nextLayers = [...layers];
            nextLayers[activeLayerIndex] = { ...nextLayers[activeLayerIndex], data: pixelData };
            setLayers(nextLayers);
            commitHistory(nextLayers);
  ...
  return (
    <div className="flex h-screen bg-[var(--bg-color)] text-[var(--text-color)] theme-transition select-none overflow-hidden">
      {showExporter && <AssetExporter onClose={() => setShowExporter(false)} />}
      ...
      <div className="hidden md:flex flex-col w-16 lg:w-24 bg-[var(--hardware-beige)] border-r-4 border-[var(--panel-shadow)] p-2 lg:p-3 items-center gap-4 z-50 shrink-0 shadow-xl">
        ...
        <SidebarToolButton active={currentTool === 'pen'} onClick={() => setCurrentTool('pen')} icon={<PenTool size={24} />} label={t.tools.pen} />
        <SidebarToolButton active={currentTool === 'eraser'} onClick={() => setCurrentTool('eraser')} icon={<Eraser size={24} />} label={t.tools.del} />
        <SidebarToolButton active={currentTool === 'fill'} onClick={() => setCurrentTool('fill')} icon={<PaintBucket size={24} />} label={t.tools.fill} />
        <SidebarToolButton active={currentTool === 'picker'} onClick={() => setCurrentTool('picker')} icon={<Pipette size={24} />} label={t.tools.pick} />
        <SidebarToolButton active={currentTool === 'pan'} onClick={() => setCurrentTool('pan')} icon={<Hand size={24} />} label={t.tools.pan} />
      ...
      <div className="flex-1 flex flex-col h-full relative min-w-0">
        <header className="p-3 lg:p-4 bg-[var(--hardware-beige)] border-b-4 border-[var(--panel-shadow)] flex justify-between items-center text-[var(--text-color)] z-40 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col md:flex-row md:items-baseline md:gap-4">
              ...
              <span className="hidden sm:inline-block label-tag !text-[8px] lg:!text-[10px]">{t.res}: {art.width}x{art.height}</span>
            </div>
          </div>
          <div className="flex gap-2">
            <div className="hidden md:flex gap-3 mr-2 border-r-2 border-[var(--panel-shadow)] pr-4">
               <button onClick={undo} disabled={historyIndex === 0} className="cassette-button px-4 py-1 text-[10px] font-black disabled:opacity-30">{t.undo}</button>
               <button onClick={redo} disabled={historyIndex === history.length - 1} className="cassette-button px-4 py-1 text-[10px] font-black disabled:opacity-30">{t.redo}</button>
            </div>
        ...
        <div className="bg-[var(--hardware-beige)]/60 backdrop-blur-md border-b-2 border-[var(--border-color)] p-1.5 flex items-center justify-center gap-2 overflow-x-auto no-scrollbar z-30 shrink-0">
          ...
          <div className="flex bg-black/10 p-1 border-2 border-[var(--border-color)]">
            <IconButton active={showGrid} onClick={() => setShowGrid(!showGrid)} icon={<Grid3X3 size={18} />} title={t.grid} />
            <IconButton active={symmetryMode !== 'none'} onClick={cycleSymmetry} icon={getSymmetryIcon(symmetryMode, 18)} title={t.sym} />
          </div>

          <div className="h-6 w-px bg-[var(--panel-shadow)] mx-1"></div>

          <div className="flex bg-black/10 p-1 border-2 border-[var(--border-color)]">
            <IconButton onClick={handleSave} icon={<Download size={18} />} title={t.save} color="text-blue-500" />
            <IconButton onClick={handleExportPNG} icon={<ImageDown size={18} />} title={t.export} color="text-orange-500" />
          </div>
        </div>
        ...
        >
          ...
          <div className="absolute bottom-4 left-6 font-mono text-[9px] text-[var(--led-green)] font-black opacity-80 drop-shadow-[0_2px_4px_black] bg-black/40 p-2 border-l-4 border-[var(--led-green)] hidden lg:block">
            <p className="tracking-widest opacity-60 uppercase">{t.status}</p>
        ...
        <div className="md:hidden bg-[var(--hardware-beige)] border-t-8 border-[var(--panel-shadow)] p-4 safe-bottom text-[var(--text-color)] z-[100]">
          ...
          <div className="flex justify-between gap-1.5 border-t-4 border-[var(--panel-shadow)] pt-4">
            <ControlDeckButton active={currentTool === 'pen'} onClick={() => setCurrentTool('pen')} icon={<PenTool size={22} />} label={t.tools.pen} />
            <ControlDeckButton active={currentTool === 'eraser'} onClick={() => setCurrentTool('eraser')} icon={<Eraser size={22} />} label={t.tools.del} />
            <ControlDeckButton active={currentTool === 'fill'} onClick={() => setCurrentTool('fill')} icon={<PaintBucket size={22} />} label={t.tools.fill} />
            <ControlDeckButton active={currentTool === 'picker'} onClick={() => setCurrentTool('picker')} icon={<Pipette size={22} />} label={t.tools.pick} />
            <ControlDeckButton active={currentTool === 'pan'} onClick={() => setCurrentTool('pan')} icon={<Hand size={22} />} label={t.tools.pan} />
          </div>
        </div>
      </div>
      ...
      <div className="hidden md:flex flex-col w-64 lg:w-80 bg-[var(--hardware-beige)] border-l-4 border-[var(--panel-shadow)] h-full p-4 lg:p-6 z-40 shrink-0 shadow-[-10px_0_30px_rgba(0,0,0,0.1)] relative overflow-y-auto">
           ...
           <div className="flex-1 flex flex-col min-h-[250px] mb-8 bg-black/5 p-4 border-2 border-[var(--panel-shadow)] shadow-inner">
              <div className="flex justify-between items-center mb-4 pb-2 border-b border-black/10">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--panel-shadow)]">{t.layers}</label>
           ...
           <div className="space-y-6 pt-6 border-t-4 border-[var(--panel-shadow)]">
              <div className="bg-[var(--hardware-dark)] p-4 border-2 border-[var(--border-color)] shadow-inner rounded-sm">
                <div className="flex justify-between items-center mb-2">
                   <span className="text-[9px] font-black text-white/50 uppercase tracking-[0.2em] font-mono">{t.brush}</span>
              ...
              <button onClick={handleSave} className="w-full bg-[var(--accent-orange)] text-white p-4 font-black uppercase border-4 border-[var(--border-color)] shadow-[4px_4px_0px_var(--border-color)] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all flex items-center justify-center gap-3 text-lg tracking-tighter">
                 ...
                 <span>{isSaving ? t.saveSuccess : t.commit}</span>
              </button>
           </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center p-6 z-[300]">
           <div className="bg-[var(--hardware-beige)] border-4 border-[var(--border-color)] p-6 w-full max-w-sm text-[var(--text-color)]">
              <div className="flex justify-between items-center mb-6 border-b-4 border-[var(--panel-shadow)] pb-2">
                 <h2 className="text-lg font-black uppercase tracking-tight">{t.layers}</h2>
           ...
           <div className="bg-[var(--hardware-beige)] border-8 border-[var(--border-color)] p-10 w-full max-w-md text-[var(--text-color)] shadow-[20px_20px_0px_rgba(0,0,0,0.5)]">
              <div className="flex justify-between items-center mb-8 border-b-8 border-[var(--panel-shadow)] pb-4">
                 <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-2xl font-black uppercase tracking-tight">{t.config}</h2>
                 </div>
              ...
              <div className="space-y-8">
                 <div>
                    <label className="block text-[10px] font-black uppercase mb-4 tracking-widest text-[var(--panel-shadow)]">{t.visualProfile}</label>
                 ...
                 <div className="pt-6 border-t-8 border-[var(--panel-shadow)]">
                    <label className="block text-[10px] font-black uppercase mb-4 tracking-widest text-[var(--panel-shadow)]">{t.lang}</label>
                    <div className="flex gap-4">
                       <button onClick={() => setLang('zh')} className={`flex-1 p-4 border-4 font-black uppercase text-sm ${lang === 'zh' ? 'bg-[var(--accent-orange)] text-white border-white' : 'bg-white border-[var(--panel-shadow)]'}`}>中文</button>
                       <button onClick={() => setLang('en')} className={`flex-1 p-4 border-4 font-black uppercase text-sm ${lang === 'en' ? 'bg-[var(--accent-orange)] text-white border-white' : 'bg-white border-[var(--panel-shadow)]'}`}>English</button>
                    </div>
                 </div>

                 <div className="grid grid-cols-1 gap-3 pt-6 border-t-8 border-[var(--panel-shadow)]">
                    <button 
                      onClick={() => layerFileInputRef.current?.click()}
                      className="w-full bg-orange-600 p-4 border-4 border-white/20 text-white font-black uppercase text-xs flex items-center justify-center gap-3 hover:bg-orange-500 transition-all"
                    >
                      <Upload size={20} className="text-white" />
                      {t.importLayer}
                    </button>
                    <input type="file" ref={layerFileInputRef} onChange={handleImportToLayer} accept="image/*" className="hidden" />
                    
                    <button 
                      onClick={() => setShowExporter(true)}
                      className="w-full bg-zinc-800 p-4 border-4 border-white/20 text-white font-black uppercase text-xs flex items-center justify-center gap-3 hover:bg-zinc-700 transition-all"
                    >
